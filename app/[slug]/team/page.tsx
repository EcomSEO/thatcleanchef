...
export async function generateMetadata({
  ...
  const url = `${SITE.url}/${locale}/team`;
  const languages: Record<string, string> = {};
  for (const l of LOCALES) {
    languages[HREFLANG[l]] = l === "en" ? `${SITE.url}/team` : `${SITE.url}/${l}/team`;
  }
  languages["x-default"] = `${SITE.url}/team`;
  return {
    title: `${strings.navTeam} — ${SITE.name}`,
    description: strings.trustReviewedDek,
...
export default async function LocalizedTeamPage({
  ...
  return (
    <LocalizedStaticTemplate
      locale={locale}
      englishHref="/team"
      eyebrow={strings.navTeam}
      title={strings.navTeam}
      intro={strings.trustReviewedDek}
    />
  );
}
